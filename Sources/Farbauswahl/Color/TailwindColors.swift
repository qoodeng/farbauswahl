...
/// Built-in Tailwind CSS v3 color palette for matching.
final class TailwindColors {
    static let shared = TailwindColors()
    ...
    private init() {
        colors = Self.palette.map { (name, hex) in
            let cv = ColorValue(hex: hex)
            return TailwindColor(name: name, color: cv, oklab: cv.oklab)
        }
    }

    func nearest(to color: ColorValue) -> Match? {
        nearestMatches(to: color, count: 1).first
    }

    func nearestMatches(to color: ColorValue, count: Int = 2) -> [Match] {
        let lab = color.oklab
        var best: [(index: Int, dist: Double)] = []

        for (i, tc) in colors.enumerated() {
            let dL = lab.L - tc.oklab.L
            let da = lab.a - tc.oklab.a
            let db = lab.b - tc.oklab.b
        ...
        return best.map { entry in
            let tc = colors[entry.index]
            return Match(name: tc.name, hex: tc.color.hex, deltaE: sqrt(entry.dist))
        }
    }

    // MARK: - Tailwind v3 default palette
